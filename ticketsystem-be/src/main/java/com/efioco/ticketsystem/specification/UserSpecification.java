...
public class UserSpecification implements Specification<UserEntity> {
	...
	public Predicate toPredicate(Root<UserEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		if (!CollectionUtils.isEmpty(filter.getRoles())) {
		    ...
		    predicati.add(rolesJoin.get("id").in(filter.getRoles()));
		    query.distinct(true); // Evita duplicati
		}

		// removed flag
		if (filter.getActive() != null) {
			predicati.add(cb.isFalse(root.get("active")));
		}

		return cb.and(predicati.toArray(new Predicate[0]));
	}

	private boolean isFilterPresent(UserDTO filter) {
